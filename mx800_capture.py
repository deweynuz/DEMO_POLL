...
# Valeurs de monitor_ip déclenchant la découverte automatique
AUTO_IP_VALUES = ('', 'auto', 'AUTO', 'Auto', None)
# Délai sans association réussie avant de relancer une découverte (secondes)
REDISCOVER_AFTER = 120
...
def run(monitor_ip: str, db_path: str, csv_dir: str, demo_json: str,
    ...
    # Defauts appliques en dernier : CLI > config.json > valeurs ci-dessous
    home          = os.path.expanduser('~')
    db_path       = db_path       or f'{home}/hegp.db'
    csv_dir       = csv_dir       or f'{home}/data/'
    demo_json     = demo_json     or f'{home}/patient_demo.json'
    hdf5_dir      = hdf5_dir      or f'{home}/waves/'
    poll_interval = poll_interval or 1.0
    demo_interval = demo_interval or 30
    ...
    auto_mode = monitor_ip in AUTO_IP_VALUES

    if auto_mode:
        if not DISCOVERY_AVAILABLE:
            log.error('monitor_ip="auto" exige discover_monitor.py a cote de ce script.')
            raise SystemExit(1)
        log.info("Recherche du moniteur (MAC Philips 00:09:FB)...")
        while True:
            found = discover_monitor()
            if found:
                monitor_ip = found
                log.info(f"Moniteur decouvert : {monitor_ip}")
                break
            log.warning("Aucun moniteur joignable — nouvelle tentative dans 30s")
            time.sleep(30)
    ...
    last_assoc_ok = time.time()

    def rediscover() -> bool:
        """Relance une decouverte. True si l'adresse a change."""
        nonlocal monitor_ip
        if not DISCOVERY_AVAILABLE:
            return False
        found = discover_monitor()
        if found and found != monitor_ip:
            log.warning(f"Moniteur deplace : {monitor_ip} -> {found}")
            monitor_ip = found
            return True
        return False
    ...
    try:
        while True:
            ...
            # Le moniteur reste injoignable : il a peut-etre change d'adresse
            if auto_mode and not associated and \
               (time.time() - last_assoc_ok) > REDISCOVER_AFTER:
                if rediscover():
                    send_assoc()
                last_assoc_ok = time.time()   # evite une boucle de decouverte
...
if __name__ == '__main__':
    ...
    # Les valeurs vides laissent la main a config.json, puis aux defauts de run().
    run(
        monitor_ip    = args.ip,
        db_path       = args.db,
        csv_dir       = args.csv,
