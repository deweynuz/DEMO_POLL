...
# Configuration BOOTP/DHCP pour moniteur Philips MX800
# Généré par install.sh — ne pas éditer manuellement

interface=eth0
bind-interfaces
except-interface=lo

# Pas de service DNS (uniquement DHCP/BOOTP)
port=0

# Plage d'adresses attribuées au moniteur
dhcp-range=$DHCP_START,$DHCP_END,255.255.255.0,12h

# IMPORTANT — Le moniteur Philips demande son adresse en BOOTP, pas en DHCP.
# dnsmasq n'active l'allocation BOOTP dynamique qu'en présence d'au moins une
# entrée dhcp-host. Cette entrée « déclencheur » pointe une MAC fictive : elle
# ne sert qu'à activer le BOOTP pour tous les moniteurs, quelle que soit leur MAC.
# NE PAS SUPPRIMER : sans elle, le moniteur n'obtient jamais d'adresse.
dhcp-host=00:00:00:00:00:01,$BOOTP_TRIGGER_IP
